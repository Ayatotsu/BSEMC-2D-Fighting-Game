...
public class HandleAnimations : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        anim.SetBool("TakesHit", states.getHit);
        anim.SetBool("OnAir", !states.onGround);
        anim.SetBool("Crouch", states.crouch);

        float movement = Mathf.Abs(states.horizontal); //absolute value when pressing left or right
        anim.SetFloat("Movement", movement);

        if (states.vertical < 0) //hold down button
        {
            states.crouch = true;
        }
        else 
